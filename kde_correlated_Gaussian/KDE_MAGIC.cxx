  ...
  VISKORES_EXEC void operator()(const T& inVector, T2& outMagnitude, const T3& isovalue) const
    {
        ...
        // Quant1 and quant 2 are specific to the Gaussian kernel
        ...
        // Kernel bandwidth with the Silverman's rule of thumb for a Gaussian kernel
        ...
        // Derive most probable sign and create relevant data grid
...
"inputEnsemble" (input): Ensemble data,
...
struct ComputeEdgeVarianceWorklet : viskores::worklet::WorkletMapField
{
                         ...
                         viskores::Float64& outputExpectedCrossing) const
  {
    ...
    // Compute mean of the ensemble data
    ...
    // Compute variance of the ensemble data
    for(int i=0; i<numKernels; i++)
    {
    ...
    for(int i=0; i<numKernels; i++)
    {
        coVar+= ((double)ensemble0[i] - s0Mean)*((double)ensemble1[i] - s1Mean);
    }
    coVar = coVar/(double)numKernels;


    float sf = pow(numKernels*(2.0 + 2.0) / 4.0, -1. / (2.0 + 4.0));
    float kernelCov = coVar* sf* sf;
    ...
    // For perfectly correlated data (as in synthetic tangle data), to avoid singular determinant of covariance matrix, following modification is needed
    /*g.kde_gaussian_alpha_pdf(ens0,
                             &dev1,
                             ens1,
                             &dev2,
                             0.99*viskores::Sqrt(dev1)*viskores::Sqrt(dev2),
                             interpMean,
                             &expectedConsiderCorr,
                             &c_probConsiderCorr,
                             &varConsiderCorr,
...
viskores::cont::DataSet KDE_MAGIC::DoExecute(const viskores::cont::DataSet& inDataSet)
{
    ...
    auto resolveType = [&](const auto& concrete)
    {
       ...
       //this->Invoke(ComputeMostProbableTopology{}, concrete, result, isovalue);
       meanArray = result;
    };
    ...
    // Use the expected isosurface crossing locations on grid edges computed with KDE MAGIC ([0,1]) to compute physical isosurface vertex locations
