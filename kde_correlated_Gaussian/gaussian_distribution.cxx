...
{
    // The number of ensemble members/kernels at each end of the edge
    int numKde1 = getNumGaussiansInKde1();
    int numKde2 = getNumGaussiansInKde2();

    // Variables to compute inverse linear interpolation probability at 100 points with array intialized to 0
...
"covarXY" (input): sample covariance of random variables X and Y,
"c" (input): Isovalue,
...
void gaussian_distribution::gaussian_alpha_pdf(float muX, float varX, float muY, float varY, float covarXY, double c, double* alphaDensity)
{
    ...
    // Compute the expected value, crossing probability, and variance of alpha distribution using the Hinkley's approach
